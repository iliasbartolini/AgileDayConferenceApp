...
/**
 * Custom view that draws a vertical time "ruler" representing the chronological progression of a single day. 
 * Usually shown along with {@link BlockView} instances to give a spatial sense of time.
 */
public class TimeRulerView extends View {
	...
	public static TimeZone CONFERENCE_TIME_ZONE = TimeZone.getTimeZone("Europe/Rome");
	...
	public TimeRulerView(Context context, AttributeSet attrs, int defStyle) {
		...
		mHorizontalDivider = a.getBoolean(R.styleable.TimeRulerView_horizontalDivider, mHorizontalDivider);
		mLabelTextSize = a.getDimensionPixelSize(R.styleable.TimeRulerView_labelTextSize, mLabelTextSize);
	...
	/**
	 * Return the vertical offset (in pixels) for a requested time (in milliseconds since epoch).
	 */
	...
	private String getFormattedLabel(int iHour) {
		// TODO: localize these labels better, including handling
		// 24-hour mode when set in framework.
		final boolean is24HourMode = true;

		final int hour = mStartHour + iHour;
